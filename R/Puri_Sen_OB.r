...
#'
#' Performs the Puri and Sen method for a single covariate using a biased
...
#'
#' @importFrom stats cor cov.wt pchisq aggregate terms as.formula
#' @export
Puri_Sen_OB <- function(data, formula) {
  ...
  V <- cov.wt(data[, c("ranked_response", "ranked_covariate")], method = "ML")$cov
  if (det(V) == 0) {
